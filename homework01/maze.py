...
class MazeProblem(Problem):
    ...
    def actions(self, state):
        ...
        if x < 1 or maze.at((x-1, y)) == "#":
            possible_actions.remove("UP")

        if x >= maze.maze_size[0] or maze.at((x + 1, y)) == "#":
            possible_actions.remove("DOWN")

        if y < 1 or maze.at((x, y-1)) == "#":
            possible_actions.remove("LEFT")

        if y >= maze.maze_size[1] or maze.at((x, y+1)) == "#":
    ...
    def result(self, state, action):
        ...
        if action == "TELEPORT" and len(maze.teleporters) > 0:
            if state.position == state.maze.teleporters[0]:
                new_position = state.maze.teleporters[1]

            if state.position == state.maze.teleporters[1]:
    ...
    def goal_test(self, state):
        ...
        # YOUR CODE HERE
        return state.maze.at(state.position) == "x"
